...
[ my-sentiment                            ;; Each trader can have a positive sentiment (+1), in which case he is 'bullish',
  ...
  trader-status                           ;; There are two groups: 1 = trader and 2 = etf trader
  etf-shares
  etf?
  ]
...
to export-results
  ...
  repeat 20 [
    ...
    let time ticks
    let mreturn returns
    let mvol volatility-indicator
    let total-shares sum [number-of-shares] of patches
    let total-etf-shares sum [etf-shares] of patches
    file-print (word time "," log-price "," mreturn "," mvol "," total-shares "," total-etf-shares)
  ]
  file-close
end
@#$#@#$#@
