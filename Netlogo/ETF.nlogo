...
to setup
  ...
  set log-price lprice-range
  set total-etf-shares 0
  set total-shares 0
  reset-ticks
end
...
to agent-decision
  ...
  ask patches
  [
    if my-sentiment = 1 [ set pcolor green]
    if my-sentiment = -1 [ set pcolor red ]
  ]
end


to update-total-shares
 set total-shares sum [number-of-shares] of patches
end
;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Form ETF
;;;;;;;;;;;;;;;;;;;;;;;;;;
...
to form-etf
  ask patches
  [
    ifelse number-of-shares > etf-threshold
    ;; condition has met
    [
      set etf? true
      set etf-shares etf-shares + 1
      set pcolor blue
    ]
...
to update-etf
  ...
  ask patches with [etf? = true]  ;; Only ask ETF patches
  [
    ;; Step 1: Calculate the decision value for this patch
    let total-sentiment 0
    if any? neighbors4 [
      set total-sentiment sum [my-sentiment] of neighbors4
    ]

    let weighted-sentiment propensity-to-sentiment-contagion * total-sentiment
    let weighted-news news-sensitivity * news-qualitative-meaning
    let random-value random-normal miu opinion-vol

    let decision-value (weighted-sentiment + weighted-news + random-value)

    ;; Step 2: Calculate the average decision value for the neighboring patches
    let total-neighbor-decision-value 0
    let neighbor-count 0

    ask neighbors4 [
      let neighbor-sentiment sum [my-sentiment] of neighbors4
      let neighbor-weighted-sentiment propensity-to-sentiment-contagion * neighbor-sentiment
      let neighbor-weighted-news news-sensitivity * news-qualitative-meaning
      let neighbor-random-value random-normal miu opinion-vol

      let neighbor-decision-value (neighbor-weighted-sentiment + neighbor-weighted-news + neighbor-random-value)
      set total-neighbor-decision-value total-neighbor-decision-value + neighbor-decision-value
      set neighbor-count neighbor-count + 1
    ]

    let average-neighbor-decision-value 0
    if neighbor-count > 0 [
      set average-neighbor-decision-value total-neighbor-decision-value / neighbor-count
    ]

    ;; Step 3: Compare the current patch's decision value to the neighbors' average
    ifelse decision-value > average-neighbor-decision-value
    [
      ;; Current patch is better than the neighbors
      set my-sentiment 1
      set number-of-shares number-of-shares + 1  ;; Buy shares if the decision is better
    ]
    [
      ;; Current patch is worse or equal to the neighbors
      set my-sentiment -1
      if number-of-shares > 0 [
        set number-of-shares number-of-shares - 1  ;; Sell shares if condition fails
        set etf-shares etf-shares - 1
      ]
    ]
  ]
...
;; This procedure calculates the sum of etf-shares across all patches
to update-total-etf-shares
  set total-etf-shares sum [etf-shares] of patches
end
;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;
...
Any references to this model for academic publication should refer to: Carlos Pedro GonÁalves (2003) Artificial Financial Market Model. http://ccl.northwestern.edu/netlogo/models/community/Artificial Financial Market Model
...
@#$#@#$#@
<experiments>
  <experiment name="RandomRuns" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="750"/>
    ...
    <metric>total-shares</metric>
    <metric>total-etf-shares</metric>
    ...
    <enumeratedValueSet variable="etf-threshold">
      <value value="20"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="lprice-range">
      <value value="0"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="eSentiment" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="750"/>
    <metric>returns</metric>
  ...
  <experiment name="eLongRun" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="Experiment ETF count" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="750"/>
    <metric>log-price</metric>
    <metric>volatility-indicator</metric>
    <metric>count patches with [ etf? ]</metric>
    <enumeratedValueSet variable="sigma">
      <value value="0.9"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max-news-sensitivity">
